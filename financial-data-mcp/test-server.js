  ...
  async handleToolRequest(toolName, args) {
    ...
    switch (toolName) {
      ...
      case 'get_stock_price_history':
        ...
        const from = (args && args.from) ? this.parseDate(args.from) : Date.now() / 1000 - 365 * 5;
        const to = (args && args.to) ? this.parseDate(args.to) : Date.now() / 1000;
...
setTimeout(async () => {
  try {
    console.error('Testing get_stock_quote...');
    const result = await finnhub.getStockQuote('AAPL');
